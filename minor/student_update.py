...
class student_page:
	...
	def __init__(self, master, *args, **kwargs):
		...
		self.h1=Label(master,text="UPDATE", font=("arial 20 bold")).place(x=400,y=0)
		
		self.a0=Label(master,text="Enrollment no. :",width=15,height=2,font=('arial',15,'bold')).place(x=350,y=60)
		self.entry=Entry(master,textvariable=self.v).place(x=530,y=80,width=190)
		self.btn_0=Button(master, text="SEARCH",command=self.search, width=12,height=2,bg='orange').place(x=750,y=60)
		...
		self.btn_1=Button(master, text="UPDATE",width=12,height=2,bg='blue',command=self.update_info).place(x=550,y=650)
	...
	def update_info(self, *args, **kwargs):
		self.u1=self.entry1.get()
		self.u2=self.entry2.get()
		self.u3=self.entry3.get()
		self.u4=self.entry4.get()
		self.u5=self.entry5.get()
		self.u6=self.entry6.get()
		self.u7=self.entry7.get()
		self.u8=self.entry8.get()
		self.u9=self.entry9.get()
		self.u10=self.entry10.get()
		self.u11=self.entry11.get()
		self.u12=self.entry12.get()
		self.u13=self.entry13.get()
		self.u14=self.entry14.get()
		self.u15=self.entry15.get()
		self.u16=self.entry16.get()
		
		query="UPDATE student_registration SET sname=?, dob=?, mname=?,fname=?,address=?,gender=?,email=?,contact=?,lname=?,relation=?, ladd=?, lcontact=?,school=?,year=?,branch=?,fee=? WHERE rollno=?"
		c.execute(query, (self.u1,self.u2,self.u3,self.u4,self.u5,self.u6,self.u7,self.u8,self.u9,self.u10,self.u11,self.u12,self.u13,self.u14,self.u15,self.u16,self.s))
		conn.commit()
		ms.showinfo("", "Update Successfully")
